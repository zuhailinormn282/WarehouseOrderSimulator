...
namespace WarehouseOrderSimulator
{
    public class OrderProcessor
    {
        ...
        public bool ProcessOrder(string orderNumber, List<OrderItem> items)
        {
            ...
            Console.WriteLine($"\n Total Amount: RM{totalAmount:F2}");
            ...
            _db.LogTransaction("ORDER_COMPLETED", null, null, orderNumber, $"Order completed successfully. Total: RM{totalAmount:F2}");
        ...
        private void PrintReceipt(string orderNumber, List<(OrderItem item, int availableStock, Product product)> items, decimal totalAmount)
        {
            const int width = 60;
            Console.WriteLine($"\n{new string('=', width)}");
            Console.WriteLine($"{"RECEIPT",(width / 2) + 3}");
            ...
            foreach (var (item, _, product) in items)
            {
                ...
                Console.WriteLine($"{product.Name,-25} {item.Quantity,-10} RM{item.UnitPrice,-10:F2} RM{lineTotal:F2}");
            }

            Console.WriteLine($"{new string('-', width)}");
            Console.WriteLine($"{"TOTAL",-49} RM{totalAmount:F2}");
            Console.WriteLine($"{new string('=', width)}");
